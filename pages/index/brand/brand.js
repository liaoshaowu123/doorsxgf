 ...
 */
  getList() {
    ...
    ljrqe.post('houseType/getBrandData', data).then(res => {
      ...
      this_.comPrice();
      type = 0;
      id = this.data.lclist[this.data.lcIndex].id;
      this.getData();
    })
  ...
  comPrice(){
    ...
    let lcPrice,lcPricesc,pjPrice,fhlPrice,blPrice,azPrice,total_price,total_pricesc;

    //铝材
    let spanLvPrice = this.data.lclist[this.data.lcIndex].seriesList[this.data.lcIndex1].price ;
    let spanLvPricesc = this.data.lclist[this.data.lcIndex].seriesList[this.data.lcIndex1].pricesc ;
    lcPrice = parseFloat((lvcai * spanLvPrice).toFixed(2));
    lcPricesc = parseFloat((lvcai * spanLvPricesc).toFixed(2));
    ...
    total_pricesc =Math.round((lcPricesc + pjPrice + fhlPrice + blPrice + azPrice)*100)/100;
    this.setData({
      lcdjPrice: spanLvPrice,
      lcdjPricesc: spanLvPricesc,
      lcPrice,
      lcPricesc,
      pjPrice,
      fhlPrice,
